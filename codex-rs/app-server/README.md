...
Experimental clients can pass `experimentalRawEvents: true` on `thread/resume` to enable raw response events for subsequent turns, including when rejoining an already-loaded thread. This does not replay raw usage events from earlier turns.
